import { Link } from '@remix-run/react';

export const Footer = () => {
  return (
    <footer className="pt-10 text-sm text-white bg-primary-950 mt-20">
      <div className="px-4 mx-auto max-w-df">
        <div className="flex flex-wrap gap-6 lg:flex-nowrap">
          <div className="flex items-center justify-center w-full h-full lg:w-1/4">
            <img src="/images/logo.png" alt="" width="156" height="105" />
          </div>
          <div className="w-full space-y-8 lg:w-1/4">
            <div>
              <h5 className="mb-4 text-lg font-bold">CONTÁCTENOS</h5>
              <ul className="[&>*>a]:flex [&>*>a]:items-center [&>*>a]:gap-2 space-y-3">
                <li>
                  <a href="#">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                      strokeWidth="1.5"
                      stroke="currentColor"
                      className="size-5"
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 0 0 2.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 0 1-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 0 0-1.091-.852H4.5A2.25 2.25 0 0 0 2.25 4.5v2.25Z"
                      />
                    </svg>
                    987654321
                  </a>
                </li>
                <li>
                  <a href="#">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                      strokeWidth="1.5"
                      stroke="currentColor"
                      className="size-5"
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75"
                      />
                    </svg>
                    ventas&#64;web.com
                  </a>
                </li>
                <li>
                  <a href="#">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                      strokeWidth="1.5"
                      stroke="currentColor"
                      className="size-5"
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"
                      />
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z"
                      />
                    </svg>
                    Calle 123 #456 Colonia
                  </a>
                </li>
              </ul>
            </div>
            <div>
              <h5 className="mb-4 text-lg font-bold">POLÍTICAS</h5>
              <ul className="[&>*>a]:flex [&>*>a]:items-center [&>*>a]:gap-2 space-y-2">
                <li>
                  <a href="#"> TÉRMINOS Y CONDICIONES </a>
                </li>
                <li>
                  <a href="#"> POLÍTICAS DE PRIVACIDAD </a>
                </li>
                <li>
                  <a href="#"> CONTACTO </a>
                </li>
                <li>
                  <a href="#"> NOSOTROS </a>
                </li>
                <li>
                  <a href="#"> SERVICIOS </a>
                </li>
              </ul>
            </div>
          </div>
          <div className="w-full space-y-8 lg:w-1/4">
            <div>
              <h5 className="mb-4 text-lg font-bold">ACEPTAMOS PAGOS CON:</h5>
              <ul className="flex items-center gap-2">
                <li>
                  <a href="#">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="size-8"
                      viewBox="0 0 2304 1536"
                    >
                      <path
                        fill="currentColor"
                        d="M1975 862h-138q14-37 66-179l3-9q4-10 10-26t9-26l12 55zM531 797l-58-295q-11-54-75-54H130l-2 13q311 79 403 336m179-349L548 886l-17-89q-26-70-85-129.5T315 579l135 510h175l261-641zm139 642h166l104-642H953zm768-626q-69-27-149-27q-123 0-201 59t-79 153q-1 102 145 174q48 23 67 41t19 39q0 30-30 46t-69 16q-86 0-156-33l-22-11l-23 144q74 34 185 34q130 1 208.5-59t80.5-160q0-106-140-174q-49-25-71-42t-22-38q0-22 24.5-38.5T1455 571q70-1 124 24l15 8zm425-16h-128q-65 0-87 54l-246 588h174l35-96h212q5 22 20 96h154zm262-320v1280q0 52-38 90t-90 38H128q-52 0-90-38t-38-90V128q0-52 38-90t90-38h2048q52 0 90 38t38 90"
                      />
                    </svg>
                  </a>
                </li>
                <li>
                  <a href="#">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="size-8"
                      viewBox="0 0 2304 1536"
                    >
                      <path
                        fill="currentColor"
                        d="M1119 213q-128-85-281-85q-103 0-197.5 40.5T478 277T369.5 439T329 636q0 104 40.5 198T478 996t162 108.5t198 40.5q153 0 281-85q-131-107-178-265.5t.5-316.5T1119 213m33 24q-126 99-172 249.5t-.5 300.5t172.5 249q127-99 172.5-249t-.5-300.5T1152 237m33-24q130 107 177.5 265.5t.5 317t-178 264.5q128 85 281 85q104 0 198-40.5T1826 996t108.5-162t40.5-198q0-103-40.5-197T1826 277t-162.5-108.5T1466 128q-153 0-281 85m741 722h7v-3h-17v3h7v17h3zm29 17h4v-20h-5l-6 13l-6-13h-5v20h3v-15l6 13h4l5-13zm-8 440v2h-5v-3h5zm0 9h3l-4-5h2l1-1q1-1 1-3t-1-3l-1-1h-9v13h3v-5h1zm-1262-68q0-19 11-31t30-12q18 0 29 12.5t11 30.5q0 19-11 31t-29 12q-19 0-30-12t-11-31m473-44q30 0 35 32h-70q5-32 35-32m356 44q0-19 11-31t29-12t29.5 12.5t11.5 30.5q0 19-11 31t-30 12q-18 0-29-12t-11-31m272 0q0-18 11.5-30.5t29.5-12.5t29.5 12.5t11.5 30.5q0 19-11.5 31t-29.5 12t-29.5-12.5t-11.5-30.5m158 72q-2 0-4-1q-1 0-3-2t-2-3q-1-2-1-4q0-3 1-4q0-2 2-4l1-1q2 0 2-1q2-1 4-1q3 0 4 1l4 2l2 4v1q1 2 1 3l-1 1v3l-1 1l-1 2q-2 2-4 2q-1 1-4 1m-1345-4h30v-85q0-24-14.5-38.5T575 1262q-32 0-47 24q-14-24-45-24q-24 0-39 20v-16h-30v135h30v-75q0-36 33-36q30 0 30 36v75h29v-75q0-36 33-36q30 0 30 36zm166 0h29v-135h-29v16q-17-20-43-20q-29 0-48 20t-19 51t19 51t48 20q28 0 43-20zm178-41q0-34-47-40l-14-2q-23-4-23-14q0-15 25-15q23 0 43 11l12-24q-22-14-55-14q-26 0-41 12t-15 32q0 33 47 39l13 2q24 4 24 14q0 17-31 17q-25 0-45-14l-13 23q25 17 58 17q29 0 45.5-12t16.5-32m130 34l-8-25q-13 7-26 7q-19 0-19-22v-61h48v-27h-48v-41h-30v41h-28v27h28v61q0 50 47 50q21 0 36-10m86-132q-29 0-48 20t-19 51q0 32 19.5 51.5t49.5 19.5q33 0 55-19l-14-22q-18 15-39 15q-34 0-41-33h101v-12q0-32-18-51.5t-46-19.5m159 0q-23 0-35 20v-16h-30v135h30v-76q0-35 29-35q10 0 18 4l9-28q-9-4-21-4m30 71q0 31 19.5 51t52.5 20q29 0 48-16l-14-24q-18 13-35 12q-18 0-29.5-12t-11.5-31t11.5-31t29.5-12q19 0 35 12l14-24q-20-16-48-16q-33 0-52.5 20t-19.5 51m245 68h30v-135h-30v16q-15-20-42-20q-29 0-48.5 20t-19.5 51t19.5 51t48.5 20q28 0 42-20zm133-139q-23 0-35 20v-16h-29v135h29v-76q0-35 29-35q10 0 18 4l9-28q-8-4-21-4m140 139h29v-190h-29v71q-15-20-43-20t-47.5 20.5t-19.5 50.5t19.5 50.5t47.5 20.5q29 0 43-20zm78-20l-2 1h-3q-2 1-4 3q-3 1-3 4q-1 2-1 6q0 3 1 5q0 2 3 4q2 2 4 3t5 1q4 0 6-1q0-1 2-2l2-1q1-1 3-4q1-2 1-5q0-4-1-6q-1-1-3-4q0-1-2-2l-2-1q-1 0-3-.5t-3-.5m360-1253v1280q0 52-38 90t-90 38H128q-52 0-90-38t-38-90V128q0-52 38-90t90-38h2048q52 0 90 38t38 90"
                      />
                    </svg>
                  </a>
                </li>
                <li>
                  <a href="#">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="size-8"
                      viewBox="0 0 2304 1536"
                    >
                      <path
                        fill="currentColor"
                        d="M119 554h89l-45-108zm621 526l74-79l-70-79H581v49h142v55H581v54zm158-78l99 110V895zm288-47q0-33-40-33h-84v69h83q41 0 41-36m289-4q0-29-42-29h-82v61h81q43 0 43-32m-278-466q0-29-42-29h-82v60h81q43 0 43-31m459 69h89l-44-108zM699 399v271h-66V458l-94 212h-57l-94-212v212H256l-25-60H96l-25 60H1l116-271h96l110 257V399h106l85 184l77-184zm556 556q0 20-5.5 35t-14 25t-22.5 16.5t-26 10t-31.5 4.5t-31.5 1t-32.5-.5t-29.5-.5v91H936l-80-90l-83 90H517V866h260l80 89l82-89h207q109 0 109 89M964 614v56H747V399h217v57H812v49h148v55H812v54zm1340 559v229q0 55-38.5 94.5T2172 1536H132q-55 0-93.5-39.5T0 1402V724h111l25-61h55l25 61h218v-46l19 46h113l20-47v47h541v-99l10-1q10 0 10 14v86h279v-23q23 12 55 18t52.5 6.5t63-.5t51.5-1l25-61h56l25 61h227v-58l34 58h182V346h-180v44l-25-44h-185v44l-23-44h-249q-69 0-109 22v-22h-172v22q-24-22-73-22H553l-43 97l-43-97H269v44l-22-44H78L0 525V134q0-55 38.5-94.5T132 0h2040q55 0 93.5 39.5T2304 134v678h-120q-51 0-81 22v-22h-177q-55 0-78 22v-22h-316v22q-31-22-87-22h-209v22q-23-22-91-22H911l-54 58l-50-58H458v378h343l55-59l52 59h211v-89h21q59 0 90-13v102h174v-99h8q8 0 10 2t2 10v87h529q57 0 88-24v24h168q60 0 95-17m-758-234q0 23-12 43t-34 29q25 9 34 26t9 46v54h-65v-45q0-33-12-43.5t-46-10.5h-69v99h-65V866h154q48 0 77 15t29 58m-277-467q0 24-12.5 44t-33.5 29q26 9 34.5 25.5t8.5 46.5v53h-65q0-9 .5-26.5t0-25t-3-18.5t-8.5-16t-17.5-8.5t-29.5-3.5h-70v98h-64V399l153 1q49 0 78 14.5t29 57.5m529 609v56h-216V866h216v56h-151v49h148v55h-148v54zm-426-682v271h-66V399zm693 652q0 86-102 86h-126v-58h126q34 0 34-25q0-16-17-21t-41.5-5t-49.5-3.5t-42-22.5t-17-55q0-39 26-60t66-21h130v57h-119q-36 0-36 25q0 16 17.5 20.5t42 4t49 2.5t42 21.5t17.5 54.5m239-50v101q-24 35-88 35h-125v-58h125q33 0 33-25q0-13-12.5-19t-31-5.5t-40-2t-40-8t-31-24T2082 947q0-39 26.5-60t66.5-21h129v57h-118q-36 0-36 25q0 20 29 22t68.5 5t56.5 26m-165-601v270h-92l-122-203v203h-132l-26-60h-134l-25 60h-75q-129 0-129-133q0-138 133-138h63v59q-7 0-28-1t-28.5-.5t-23 2t-21.5 6.5t-14.5 13.5t-11.5 23t-3 33.5q0 38 13.5 58t49.5 20h29l92-213h97l109 256V400h99l114 188V400z"
                      />
                    </svg>
                  </a>
                </li>
                <li>
                  <a href="#">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="size-8"
                      viewBox="0 0 36 24"
                    >
                      <path
                        fill="currentColor"
                        d="M2.4 24A2.4 2.4 0 0 1 0 21.6V2.4A2.4 2.4 0 0 1 2.4 0h31.2A2.4 2.4 0 0 1 36 2.4v19.2a2.4 2.4 0 0 1-2.4 2.4zM9.741 5.6A8.557 8.557 0 0 0 7.2 11.848v-.008a8.355 8.355 0 0 0 2.526 5.99l.002.002a8.558 8.558 0 0 0 6.053 2.497h4.123a9.17 9.17 0 0 0 6.211-2.501l-.005.004a8.173 8.173 0 0 0 2.707-5.987v-.068a8.353 8.353 0 0 0-2.714-6.172l-.006-.006a9.003 9.003 0 0 0-2.796-1.748l-.061-.021a9.135 9.135 0 0 0-3.366-.631h-4.145a8.642 8.642 0 0 0-5.992 2.403zm-1.726 6.145a7.84 7.84 0 1 1 15.68-.006a7.84 7.84 0 0 1-15.68.009zm9.618 4.64c1.884-.734 3.196-2.534 3.196-4.64s-1.311-3.906-3.162-4.628l-.034-.012zm-6.746-4.64a4.975 4.975 0 0 0 3.154 4.628l.034.012V7.108c-1.879.739-3.186 2.537-3.188 4.64z"
                      />
                    </svg>
                  </a>
                </li>
              </ul>
            </div>
            <div>
              <h5 className="mb-4 text-lg font-bold uppercase">
                Redes sociales
              </h5>
              <ul className="flex items-center gap-2.5">
                <li>
                  <a href="#">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="20"
                      height="20"
                      viewBox="0 0 24 24"
                    >
                      <g fill="none">
                        <g clipPath="url(#IconifyId18d90225a2f4de26e0)">
                          <path
                            fill="currentColor"
                            fillRule="evenodd"
                            d="M0 12.067C0 18.034 4.333 22.994 10 24v-8.667H7V12h3V9.333c0-3 1.933-4.666 4.667-4.666c.866 0 1.8.133 2.666.266V8H15.8c-1.467 0-1.8.733-1.8 1.667V12h3.2l-.533 3.333H14V24c5.667-1.006 10-5.966 10-11.933C24 5.43 18.6 0 12 0S0 5.43 0 12.067"
                            clipRule="evenodd"
                          />
                        </g>
                        <defs>
                          <clipPath id="IconifyId18d90225a2f4de26e0">
                            <path fill="#fff" d="M0 0h24v24H0z" />
                          </clipPath>
                        </defs>
                      </g>
                    </svg>
                  </a>
                </li>
                <li>
                  <a href="#">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="20"
                      height="20"
                      viewBox="0 0 16 16"
                    >
                      <path
                        fill="currentColor"
                        d="M8 0C5.829 0 5.556.01 4.703.048C3.85.088 3.269.222 2.76.42a3.9 3.9 0 0 0-1.417.923A3.9 3.9 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7C.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297c.04.852.174 1.433.372 1.942c.205.526.478.972.923 1.417c.444.445.89.719 1.416.923c.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.9 3.9 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417c.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.9 3.9 0 0 0-.923-1.417A3.9 3.9 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046c.78.035 1.204.166 1.486.275c.373.145.64.319.92.599s.453.546.598.92c.11.281.24.705.275 1.485c.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.5 2.5 0 0 1-.599.919c-.28.28-.546.453-.92.598c-.28.11-.704.24-1.485.276c-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.5 2.5 0 0 1-.92-.598a2.5 2.5 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485c-.038-.843-.046-1.096-.046-3.233s.008-2.388.046-3.231c.036-.78.166-1.204.276-1.486c.145-.373.319-.64.599-.92s.546-.453.92-.598c.282-.11.705-.24 1.485-.276c.738-.034 1.024-.044 2.515-.045zm4.988 1.328a.96.96 0 1 0 0 1.92a.96.96 0 0 0 0-1.92m-4.27 1.122a4.109 4.109 0 1 0 0 8.217a4.109 4.109 0 0 0 0-8.217m0 1.441a2.667 2.667 0 1 1 0 5.334a2.667 2.667 0 0 1 0-5.334"
                      />
                    </svg>
                  </a>
                </li>
                <li>
                  <a href="#">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="20"
                      height="20"
                      viewBox="0 0 16 16"
                    >
                      <path
                        fill="currentColor"
                        d="M8.051 1.999h.089c.822.003 4.987.033 6.11.335a2.01 2.01 0 0 1 1.415 1.42c.101.38.172.883.22 1.402l.01.104l.022.26l.008.104c.065.914.073 1.77.074 1.957v.075c-.001.194-.01 1.108-.082 2.06l-.008.105l-.009.104c-.05.572-.124 1.14-.235 1.558a2.01 2.01 0 0 1-1.415 1.42c-1.16.312-5.569.334-6.18.335h-.142c-.309 0-1.587-.006-2.927-.052l-.17-.006l-.087-.004l-.171-.007l-.171-.007c-1.11-.049-2.167-.128-2.654-.26a2.01 2.01 0 0 1-1.415-1.419c-.111-.417-.185-.986-.235-1.558L.09 9.82l-.008-.104A31 31 0 0 1 0 7.68v-.123c.002-.215.01-.958.064-1.778l.007-.103l.003-.052l.008-.104l.022-.26l.01-.104c.048-.519.119-1.023.22-1.402a2.01 2.01 0 0 1 1.415-1.42c.487-.13 1.544-.21 2.654-.26l.17-.007l.172-.006l.086-.003l.171-.007A100 100 0 0 1 7.858 2zM6.4 5.209v4.818l4.157-2.408z"
                      />
                    </svg>
                  </a>
                </li>
              </ul>
            </div>
          </div>
          <div className="w-full lg:w-1/4">
            <iframe
              title="Google maps"
              src="https://www.google.com/maps/embed?pb=!1m14!1m12!1m3!1d15846.911984717286!2d-79.84083513465578!3d-6.8029077132005495!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!5e0!3m2!1ses-419!2spe!4v1707520859252!5m2!1ses-419!2spe"
              className="!w-full"
              width="600"
              height="270"
              style={{ border: '0' }}
              allowFullScreen
              loading="lazy"
              referrerPolicy="no-referrer-when-downgrade"
            ></iframe>
          </div>
        </div>
      </div>
      <div className="bg-[#14183e] py-2.5 text-sm mt-10">
        <div className="px-4 mx-auto max-w-df">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <p> MG COMPUTER - © 2024 TODOS LOS DERECHOS RESERVADOS</p>
            <ul className="flex items-center space-x-2">
              <li>
                <Link to="/">Inicio</Link>
              </li>
              <li>
                <Link to="/">Mi Cuenta</Link>
              </li>
              <li>
                <Link to="/">Tienda</Link>
              </li>
              <li>
                <Link to="/">Lista de deseos</Link>
              </li>
            </ul>
          </div>
        </div>
      </div>
    </footer>
  );
};
